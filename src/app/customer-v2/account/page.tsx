...
export default function AccountPage() {
  ...
  const [activeSection, setActiveSection] = useState<"profile" | "addresses" | "orders" | "help">("profile")
  ...
  useEffect(() => {
    ...
    if (section === "profile" || section === "addresses" || section === "orders" || section === "help") {
    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 flex items-center gap-2 text-3xl font-bold text-[#8D4925]" style={{ fontFamily: "var(--font-v2-playfair)" }}>
            {sectionTitle}
            {isAdmin && (
              <span
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
        <aside className="w-full lg:w-72">
          <p className="mb-4 px-4 text-xs font-bold uppercase tracking-[0.16em] text-gray-400">Settings</p>
        ...
        <section className="min-w-0 flex-1 space-y-8 lg:pt-9">
          {activeSection === "profile" ? (
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="name" className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Full Name</Label>
                <Input id="name" value={form.name} onChange={(e) => handleChange("name", e.target.value)} disabled={!isEditing} className={readOnlyFieldClasses} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="email" className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Email Address</Label>
                <Input id="email" type="email" value={form.email ?? ""} onChange={(e) => handleChange("email", e.target.value)} disabled={!isEditing} className={readOnlyFieldClasses} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="phone" className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Phone Number</Label>
                <Input id="phone" value={form.primary_mobile} disabled className={readOnlyFieldClasses} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="altPhone" className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Alternate Mobile</Label>
                <Input id="altPhone" value={form.alternative_mobile ?? ""} onChange={(e) => handleChange("alternative_mobile", e.target.value)} disabled={!isEditing} className={readOnlyFieldClasses} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="recipient" className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Recipient Name</Label>
                <Input id="recipient" value={form.recipient_name} onChange={(e) => handleChange("recipient_name", e.target.value)} disabled={!isEditing} className={readOnlyFieldClasses} />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-bold uppercase tracking-[0.12em] text-gray-400">Payment Frequency</Label>
          ...
          {activeSection === "addresses" ? (
            <div className="rounded-3xl border border-[#8D4925]/10 bg-white p-6 md:p-8">
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-lg font-semibold text-[#463028]">Default Address</CardTitle>
                {defaultAddress ? (
                  <Button variant="ghost" size="sm" onClick={() => openEditAddressModal(defaultAddress)}>
                    <PencilLine className="mr-2 h-4 w-4" />
                    Edit
                  </Button>
                ) : null}
              ...
              <CardContent className="space-y-3">
                <div className="rounded-lg border border-brand-subtle bg-white p-4 text-sm">
                  ...
                  <p className="mt-1 text-[#8d6e63]">
                    {[profile.house_apartment_no, profile.written_address, profile.city, profile.pin_code].filter(Boolean).join(", ")}
                  </p>
                </div>
                {mapEmbedUrl ? (
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-lg font-semibold text-[#463028]">Address Book</CardTitle>
              ...
              <CardContent className="space-y-3">
                {otherAddresses.length === 0 ? (
                  ...
                  otherAddresses.slice(0, 3).map((address) => (
                    <div key={address.address_id} className="rounded-lg border border-brand-subtle bg-white p-3 text-xs">
                      <p className="font-semibold text-[#463028]">{address.address_type}</p>
                      <p className="mt-1 text-[#8d6e63]">
                        {[address.house_apartment_no, address.written_address, address.city, address.pin_code].filter(Boolean).join(", ")}
                      </p>
                      <div className="mt-2 flex gap-2">
                        <Button size="sm" variant="secondary" onClick={() => openEditAddressModal(address)}>Edit</Button>
                        <Button size="sm" variant="outline" onClick={() => handleSetDefaultAddress(address.address_id)}>Make Default</Button>
                      </div>
                    </div>
                  ))
                )}
          ...
          {activeSection === "orders" ? (
            ...
            <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <Button variant="outline" onClick={() => { setOrderSearch(""); setOrderStatusFilter("all") }}>
                Reset filters
              </Button>
            </div>
                    ...
                    {filteredOrders.map((order) => {
                      ...
                      return (
                        <tr key={order.order_id} className="hover:bg-[#FAF7F0]/70">
                          <td className="px-4 py-4 font-semibold text-[#463028]">#{order.order_id}</td>
                          <td className="px-4 py-4 text-gray-600">{order.created_at ? formatDate(new Date(order.created_at), "PPP") : "Scheduled"}</td>
                          <td className="px-4 py-4">
                            <Badge variant="outline" className={`${badgeClass} border-0`}>
                              {order.status}
                            </Badge>
                          </td>
                          <td className="px-4 py-4 text-gray-600">{order.order_type ?? "one_time"}</td>
                          <td className="px-4 py-4 text-gray-600">
                            {order.items.slice(0, 2).map((item) => item.item_name).join(", ")}
                            {order.items.length > 2 ? "..." : ""}
                          </td>
                          <td className="px-4 py-4 text-right font-semibold text-[#8D4925]">{formatCurrency(order.total_price)}</td>
          ...
          {activeSection === "help" ? (
            <div className="rounded-3xl border border-[#8D4925]/10 bg-white p-6 md:p-8">
              ...
              <a href="#" className="rounded-2xl border border-[#8D4925]/10 bg-white p-4 text-sm font-medium text-gray-700 transition-colors hover:bg-[#FAF7F0]">
                Chat on WhatsApp
              </a>
              <a href="#" className="rounded-2xl border border-[#8D4925]/10 bg-white p-4 text-sm font-medium text-gray-700 transition-colors hover:bg-[#FAF7F0]">
                Email Support
