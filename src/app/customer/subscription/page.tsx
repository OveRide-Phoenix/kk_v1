...
import { Home, ShoppingBag, ShoppingCart, User, MapPin, ClipboardList } from "lucide-react"
...
export default function Subscription() {
  ...
  const [selectedSection, setSelectedSection] = useState<'new' | 'active' | null>(null)
  ...
  return (
    <div className="min-h-screen bg-[#f9f3e8]">
      ...
      <main className="container mx-auto px-4 py-8 min-h-[calc(100vh-88px)] flex flex-col">
        <h1 className="text-3xl font-bold text-[#5d4037] text-center">Your Subscription Plans</h1>
        
        <div className={`transition-all duration-500 flex items-center justify-center ${
          selectedSection ? 'mt-8' : 'flex-1'
        }`}>
          <div className="flex justify-center gap-12">
            <Button 
              variant="ghost" 
              className={`px-4 py-2 hover:bg-transparent hover:text-[#8d6e63] text-[#5d4037] font-semibold text-lg transition-all duration-200 ${
                selectedSection === 'new' ? 'text-[#8d6e63] underline underline-offset-8 decoration-2' : ''
              }`}
            ...
            <Button 
              variant="ghost" 
              className={`px-4 py-2 hover:bg-transparent hover:text-[#8d6e63] text-[#5d4037] font-semibold text-lg transition-all duration-200 ${
                selectedSection === 'active' ? 'text-[#8d6e63] underline underline-offset-8 decoration-2' : ''
              }`}
        ...
        <div className={`transition-all duration-500 mt-6 ${
          selectedSection ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}>
          {selectedSection === 'new' && (
            <div className="max-w-5xl mx-auto">
              <div className="flex justify-center">
                ...
                <div className="w-[400px]">
                  <label className="text-lg font-semibold text-[#5d4037] text-center block mb-4">
                      ...
                      <div className="text-lg font-medium text-[#5d4037] mb-4">
                        {date?.from && date?.to && (
                          <div className="flex items-center">
                            <span>
                              {format(date.from, "EEE, MMM d")} - {format(date.to, "EEE, MMM d")}
                            </span>
                          </div>
                        )}
                      </div>
                      <div className="space-y-3">
                        {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map(
                          (day) => (
                            <label
                              key={day}
                              className="flex items-center space-x-3 p-3 hover:bg-[#8d6e63]/10 rounded-lg cursor-pointer"
                            >
                              <input
                                type="checkbox"
                                className="rounded border-[#8d6e63] text-[#5d4037] focus:ring-[#8d6e63]"
                              />
          ...
          {selectedSection === 'active' && (
            <div className="max-w-4xl mx-auto">
              <h2 className="text-2xl font-semibold text-[#5d4037] mb-6">Active Subscriptions</h2>
              {/* Add your active subscriptions content here */}
            </div>
          )}
        </div>
