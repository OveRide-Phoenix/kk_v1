...
export default function ProductManagement() {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim() !== "") {
      filtered = filtered.filter((product) => {
        ...
        return (
          (name && String(name).toLowerCase().includes(searchLower)) ||
  ...
  const confirmDelete = async () => {
    ...
    if (activeTab === "combos" && (productToDelete as ComboProduct).combo_id) {
      ...
      try {
        const response = await http.delete(`/api/products/combos/${comboId}`);
        if (!response.ok) {
        ...
        toast({
          title: "Delete failed",
          description:
            error instanceof Error ? error.message : "Unexpected error while deleting combo.",
          variant: "destructive",
        });
      } finally {
    ...
    if (activeTab === "categories" && (productToDelete as CategoryProduct).category_id) {
      ...
      try {
        const response = await http.delete(`/api/products/categories/${categoryId}`);
        if (!response.ok) {
        ...
        toast({
          title: "Delete failed",
          description:
            error instanceof Error ? error.message : "Unexpected error while deleting category.",
          variant: "destructive",
        });
      } finally {
        setIsDeleteDialogOpen(false);
    ...
    if (
      ...
      (productToDelete as ComponentTypeProduct).component_type_id
    ) {
    ...
    if (activeTab === "plated" && (productToDelete as PlatedProduct).item_id) {
      ...
      try {
        const response = await http.delete(`/api/products/plated-items/${itemId}`);
        if (!response.ok) {
        ...
        toast({
          title: "Delete failed",
          description:
            error instanceof Error ? error.message : "Unexpected error while deleting plated item.",
          variant: "destructive",
        });
      } finally {
        setIsDeleteDialogOpen(false);
  ...
  const saveCombo = async (payload: ComboFormValues) => {
    ...
    const path = isUpdating ? `/api/products/combos/${payload.combo_id}` : "/api/products/combos";
    const body = {
    ...
    try {
      ...
      toast({
        title: "Combo save failed",
        description:
          error instanceof Error ? error.message : "Unexpected error while saving combo.",
        variant: "destructive",
      });
    }
  };
  ...
  const savePlatedItem = async (payload: PlatedItemFormValues) => {
    ...
    const path = isUpdating
      ? `/api/products/plated-items/${payload.item_id}`
      : "/api/products/plated-items";
    ...
    try {
      ...
      toast({
        title: "Plated item save failed",
        description:
          error instanceof Error ? error.message : "Unexpected error while saving plated item.",
        variant: "destructive",
      });
    }
  };
  ...
  const saveCategory = async (payload: CategoryFormValues) => {
    ...
    const path = isUpdating
      ? `/api/products/categories/${payload.category_id}`
    ...
    try {
      ...
      toast({
        title: "Category save failed",
        description:
          error instanceof Error ? error.message : "Unexpected error while saving category.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSaveProduct = async (product: any) => {
    ...
    try {
      const path = isCreating ? "/api/products/items" : `/api/products/items/${itemId}`;
      const response = await (isCreating ? http.post(path, payload) : http.put(path, payload));
  ...
  const selectedComponentType =
    activeTab === "component-types" &&
    selectedProduct &&
    (selectedProduct as ComponentTypeProduct).component_type_id
        ...
        {isFormOpen && activeTab === "combos" ? (
          ...
          >
            <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {selectedProduct ? "Edit Plated Item" : "Create Plated Item"}
                </DialogTitle>
              </DialogHeader>
              <PlatedItemForm
